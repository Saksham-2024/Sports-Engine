    ...
    def process_segment(self, seg_df, seg_id, match_id):
        ...
        if large_bands:
            ...
            for i, seg in enumerate(segments_split):
                seg.loc[:,'segment_idx'] = f"{seg_id}_split{i}"
...
class CleanDatasetPipeline:
    """Main preprocessing pipeline."""
    
    def __init__(self):
        self.path_resolver = PathResolver()
        self.logger = get_logger(__name__)
        self.input_file = self.path_resolver.get_path("dataset_creation.transformer_dataset_csv")
        self.output_file = self.path_resolver.get_path("tranSPORTmer.data.cleaned_csv")
        self.stats_file = self.path_resolver.get_path("tranSPORTmer.data.cleaning_stats")
    ...
    def _run(self):
        ...
        # Load
        self.logger.info("STEP 1: Cleaning XY Anomalies")
        self.logger.info(f"\n[1/3] Loading data from {self.input_file}...")
        df = pd.read_csv(self.input_file)
        self.logger.info(f"      Loaded {len(df):,} rows × {len(df.columns)} columns")
        
        # Process
        self.logger.info(f"\n[2/3] Cleaning XY anomalies...")
        self.logger.info(f"      Strategy: interpolate ≤5 | mask 5-10 | split >10 frames")
        self.handler = XYAnomalyHandler(
            x_bounds=(-1, 6.5),
            y_bounds=(-1, 14.5),
            interp_threshold=5,
            mask_threshold=10
        )
        df_cleaned, stats = self.handler.process_dataframe(df)
        
        self.logger.info(f"\n      Results:")
        self.logger.info(f"        Segments processed: {stats['segments_processed']}")
        self.logger.info(f"        Anomalies interpolated: {stats['anomalies_interpolated']} "
            f"({stats['frames_interpolated']} frames)")
        self.logger.info(f"        Anomalies masked: {stats['anomalies_masked']} "
            f"({stats['frames_masked']} frames)")
        self.logger.info(f"        Segments split: {stats['segments_split']} "
            f"({stats['segments_created']} new segments created)")
        
        # Validate
        self.logger.info(f"\n[3/3] Saving cleaned data to {self.output_file}...")
        df_cleaned.to_csv(self.output_file, index=False)
        self.logger.info(f"      ✓ Saved {len(df_cleaned):,} rows")
