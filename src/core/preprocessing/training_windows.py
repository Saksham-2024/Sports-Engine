...
class FormTrainingWindows:
    ...
    def save_windows(self, windows, output_path):
        dataset = {
    ...
    def _run(self):
        self.input_file = self.resolver.get_path("tranSPORTmer.data.normalized_csv")
        self.output_dir = self.resolver.get_path("tranSPORTmer.data.training_data_dir")
        os.makedirs(self.output_dir, exist_ok=True)
        
        self.logger.info(" CREATING TRAINING WINDOWS")
        
        df_norm = pd.read_csv(self.input_file)
        self.generator = TrainingWindowGenerator(window_size=125, target_size=35, stride=25)
        all_windows = self.generator.create_all_windows(df_norm)
        train_windows, val_windows, test_windows, split_info = self.generator.split_train_val_test(all_windows)
        
        self.logger.info(f"Train: {split_info['train_count']:,} | Val: {split_info['val_count']:,} | Test: {split_info['test_count']:,}")
        
        self.save_windows(train_windows, os.path.join(self.output_dir, 'train_windows.pkl'))
        self.save_windows(val_windows, os.path.join(self.output_dir, 'val_windows.pkl'))
        self.save_windows(test_windows, os.path.join(self.output_dir, 'test_windows.pkl'))
        
        with open(os.path.join(self.output_dir, 'split_info.json'), 'w') as f:
            json.dump({'split_info': split_info, 'window_stats': self.generator.stats}, f, indent=2)
