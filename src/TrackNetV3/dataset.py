...
class Video_IterableDataset(IterableDataset):
    ...
    def __init__(self,
        ...
        """ Initialize the dataset
            Args:
                ...
                video_range (Tuple[int]): Range of start frame and end frame of the video (frame-based, not seconds).
        ...
        # Frame-based bounds: only process frames in [start_frame, end_frame)
        if video_range is not None:
            self.start_frame = max(0, video_range[0])
            self.end_frame = min(video_range[1], self.video_len)
        else:
            self.start_frame = 0
            self.end_frame = self.video_len
    ...
    def __seek_to_frame__(self, target_frame):
        """ Drift-proof seek: seek slightly before target, then grab forward to land exactly. """
        seek_to = max(0, target_frame - 30)
        self.cap.set(cv2.CAP_PROP_POS_FRAMES, seek_to)
        while int(self.cap.get(cv2.CAP_PROP_POS_FRAMES)) < target_frame:
            if not self.cap.grab():
                break

    def __iter__(self):
        """ Return the data sequentially, only within [start_frame, end_frame). """
        # Drift-proof seek to start_frame
        self.__seek_to_frame__(self.start_frame)

        success = True
        start_f_id = self.start_frame
        end_f_id = self.start_frame
        frame_list = []
        while success:
            # Sample frames, stop at end_frame
            while len(frame_list) < self.seq_len:
                if end_f_id >= self.end_frame:
                    success = False
                    break
    ...
    def __gen_median__(self, max_sample_num, video_range):
        """ Generate the median image using fast sequential reading.

            Uses grab()/skip instead of random cap.set() seeks to avoid
            the ~100ms-per-seek penalty on compressed video files.

            Args:
                max_sample_num (int): Maximum number of frames to sample for generating median image.
                video_range (Tuple[int]): Range of start frame and end frame (frame-based, not seconds).
        """
        print('Generate median image...')
        if video_range is None:
            start_frame, end_frame = 0, self.video_len
        else:
            start_frame = max(0, video_range[0])
            end_frame = min(video_range[1], self.video_len)
        ...
        # Seek to start_frame once, then read sequentially
        self.cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame)
        
        frame_list = []
        frame_idx = start_frame
        while frame_idx < end_frame and len(frame_list) < max_sample_num:
            if (frame_idx - start_frame) % sample_step == 0:
                # Decode this frame (we want it)
                success, frame = self.cap.read()
                if not success:
                    break
                frame_list.append(frame)
            else:
                # Skip without decoding (~0.05ms vs ~2ms for read)
                if not self.cap.grab():
                    break
            frame_idx += 1
        
        if not frame_list:
            print('Warning: No frames read for median. Using black frame.')
            return np.zeros((self.HEIGHT, self.WIDTH, 3))
        
        print(f'Median: sampled {len(frame_list)} frames from [{start_frame}, {end_frame})')
