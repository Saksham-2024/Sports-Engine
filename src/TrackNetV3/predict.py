...
# Device initialized in __main__ after CUDA_VISIBLE_DEVICES is set by caller
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_range', type=lambda splits: [int(s) for s in splits.split(',')], default=None, help='range of start frame and end frame of the video (frame-based)')
    ...
    parser.add_argument('--median_file', type=str, default=None, help='file path to load or save the precomputed median array')
    parser.add_argument('--segments_file', type=str, default=None, help='json file containing gameplay segments (reserved for future use)')
    args = parser.parse_args()

    # Initialize device AFTER env setup so CUDA_VISIBLE_DEVICES from caller is respected
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"[predict.py] Using device: {device} | CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', 'not set')}")

    # Cap num_workers to prevent multiprocessing hangs
    ...
    # Load model
    print(f"[predict.py] Loading TrackNet from {args.tracknet_file}")
    ...
    print(f"[predict.py] TrackNet loaded (seq_len={tracknet_seq_len}, bg_mode={bg_mode})")

    # Handle Median Image caching
    median_img = None
    if bg_mode and args.median_file:
        if os.path.exists(args.median_file):
            print(f'[predict.py] Loading cached median image from {args.median_file}')
            median_img = np.load(args.median_file)['median']
        else:
            print(f'[predict.py] Generating median image and caching to {args.median_file}')
            dummy_ds = Video_IterableDataset(video_file, seq_len=1, sliding_step=1, bg_mode=bg_mode, max_sample_num=500, video_range=None)
            median_img = dummy_ds.median
            os.makedirs(os.path.dirname(args.median_file), exist_ok=True)
            np.savez_compressed(args.median_file, median=median_img)
            print(f'[predict.py] Median image saved to {args.median_file}')
    ...
    if args.inpaintnet_file:
        ...
        print(f"[predict.py] InpaintNet loaded (seq_len={inpaintnet_seq_len})")
    else:
        inpaintnet = None
    ...
    print(f"[predict.py] Video resolution: {w}x{h}, range: {video_range}")
    ...
    if args.eval_mode == 'nonoverlap':
        ...
        if large_video:
            dataset = Video_IterableDataset(video_file, seq_len=seq_len, sliding_step=seq_len, bg_mode=bg_mode, 
                                            max_sample_num=args.max_sample_num, video_range=video_range, median=median_img)
            data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, drop_last=False)
            print(f'[predict.py] Video length: {dataset.video_len}, processing frames [{dataset.start_frame}, {dataset.end_frame})')
        else:
        ...
        if large_video:
            dataset = Video_IterableDataset(video_file, seq_len=seq_len, sliding_step=1, bg_mode=bg_mode,
                                            max_sample_num=args.max_sample_num, video_range=video_range, median=median_img)
            data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, drop_last=False)
            video_len = dataset.end_frame - dataset.start_frame
            print(f'[predict.py] Video length: {dataset.video_len}, processing frames [{dataset.start_frame}, {dataset.end_frame}), segment_len={video_len}')
    ...
    print(f"[predict.py] Writing {len(pred_dict['Frame'])} predictions to {out_csv_file}")
